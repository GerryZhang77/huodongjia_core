...
const mockEvents = [
  {
    ...
    category: '职业发展',
    tags: ['产品设计', '用户体验', '行业交流'],
    cover_image: 'https://trae-api-sg.mchost.guru/api/ide/v1/text_to_image?prompt=product%20manager%20meetup%20professional%20networking&image_size=landscape_16_9',
    location: '北京市朝阳区',
  ...
  },
  {
    ...
    category: '技术交流',
    tags: ['前端开发', 'React', 'Vue'],
    cover_image: 'https://trae-api-sg.mchost.guru/api/ide/v1/text_to_image?prompt=tech%20meetup%20frontend%20development%20coding&image_size=landscape_16_9',
    location: '上海市浦东新区',
 ...
 */
router.get('/event-detail/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params
 ...
 */
router.get('/event-participants/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const authHeader = req.headers.authorization
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      res.status(401).json({
    ...
    res.status(200).json({
      ...
      statistics: {
        total: mockParticipants.length,
        confirmed: mockParticipants.filter(p => p.status === 'confirmed').length,
        waitlist: mockParticipants.filter(p => p.status === 'waitlist').length,
        cancelled: mockParticipants.filter(p => p.status === 'cancelled').length
      }
    })
  } catch (error) {
    res.status(500).json({
 ...
 */
router.put('/update-event/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const authHeader = req.headers.authorization
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      res.status(401).json({
 ...
 */
router.delete('/delete-event/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const authHeader = req.headers.authorization
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      res.status(401).json({
 ...
 */
router.post('/import-participants', async (req: Request, res: Response): Promise<void> => {
  try {
    const authHeader = req.headers.authorization
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      res.status(401).json({
 ...
 */
router.get('/export-participants/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const authHeader = req.headers.authorization
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      res.status(401).json({
 ...
 */
router.put('/update-participant-status', async (req: Request, res: Response): Promise<void> => {
  try {
    const authHeader = req.headers.authorization
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      res.status(401).json({
