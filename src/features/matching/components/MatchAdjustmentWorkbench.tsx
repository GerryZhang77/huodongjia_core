...
import type { MatchConstraints, ParticipantMatchResult } from "../types";

type Participant = { id: string; name: string; avatar?: string; occupation?: string; registrationTypeName?: string };
...
interface Props { activityId: string; matchResults: ParticipantMatchResult[]; participants: Participant[]; constraints: MatchConstraints; onResultsChanged: () => Promise<void> | void }

const MatchAdjustmentWorkbench: React.FC<Props> = ({ activityId, matchResults, participants, constraints, onResultsChanged }) => {
  const participantMap = useMemo(() => new Map(participants.map((person) => [person.id, person])), [participants]);
  ...
  const visiblePeople = useMemo(() => {
    ...
    return participants.filter((person) => !query || person.name.toLowerCase().includes(query) || (person.occupation || "").toLowerCase().includes(query));
  }, [participants, personQuery]);
  ...
  }, [activeRelation, candidateQuery, participants, relationEdits, resultMap]);
