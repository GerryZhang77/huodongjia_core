...
const ActivityDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      Promise.all([fetchActivityDetail(), fetchParticipants()])
  ...
  const getParticipationRate = () => {
    ...
    return Math.round((activity.current_participants / activity.max_participants) * 100)
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white">
        ...
        >
          ...
          <Tabs.Tab title={`参与者 (${participants.length})`} key="participants">
                ...
                {participants.map((participant) => (
                  <List.Item
                    ...
                    extra={
                      <Badge
                        content={
                          participant.status === 'confirmed' ? '已确认' :
                          participant.status === 'waitlist' ? '候补' : '已取消'
                        }
                        color={
                          participant.status === 'confirmed' ? '#52c41a' :
                          participant.status === 'waitlist' ? '#faad14' : '#ff4d4f'
                        }
                      />
                    }
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        报名时间：{formatDateTime(participant.registration_time)}
                      </div>
                    </div>
                  </List.Item>
                ))}
