...
const ActivityEdit = () => {
  ...
  const { activities, currentActivity, setCurrentActivity, updateActivity } = useAppStore()
    ...
    if (id) {
      ...
      if (activity) {
        ...
        form.setFieldsValue({
          ...
          registrationStart: new Date(activity.registrationStart),
          registrationEnd: new Date(activity.registrationEnd),
        ...
        // 设置封面图片
        setCoverImage([{ url: activity.coverImage }])
      } else {
        Toast.show({
          icon: 'fail',
          content: '活动不存在'
  ...
  }, [id, activities, setCurrentActivity, navigate, form])
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      navigate(`/activities/${currentActivity.id}`)
    } catch (error) {
      Toast.show({
        icon: 'fail',
        content: '更新失败，请重试'
