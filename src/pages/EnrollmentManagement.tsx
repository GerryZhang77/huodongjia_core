...
const EnrollmentManagement: React.FC = () => {
  ...
  // 更新参与者状态
  const updateParticipantStatus = async (userId: string, status: 'confirmed' | 'waitlist' | 'cancelled') => {
    try {
      ...
      actions: [
        {
          text: '确认参与',
          key: 'confirm',
          onClick: () => updateParticipantStatus(participant.user_id, 'confirmed')
        },
        {
          text: '加入候补',
          key: 'waitlist',
          onClick: () => updateParticipantStatus(participant.user_id, 'waitlist')
        },
        {
          text: '取消报名',
          key: 'cancel',
          danger: true,
          onClick: () => updateParticipantStatus(participant.user_id, 'cancelled')
        }
      ],
      cancelText: '取消'
    })
  ...
  const tabItems = [
    ...
    { key: 'confirmed', title: `已确认 (${participants.filter(p => p.status === 'confirmed').length})` },
    { key: 'waitlist', title: `候补 (${participants.filter(p => p.status === 'waitlist').length})` },
    { key: 'cancelled', title: `已取消 (${participants.filter(p => p.status === 'cancelled').length})` },
  ]
      ...
      <div className="bg-white p-4 border-b border-gray-100">
        ...
        {isUploading && (
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-500 mt-1">正在导入参与者数据... {uploadProgress}%</p>
          </div>
        )}
                ...
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedParticipants(prev => [...prev, participant.user_id])
                            } else {
                              setSelectedParticipants(prev => prev.filter(id => id !== participant.user_id))
                            }
                          }}
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500 pt-2 border-t border-gray-100">
                      <span>报名时间：{formatDateTime(participant.registration_time)}</span>
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <div className="flex space-x-4">
              ...
              <span className="text-yellow-600">
                候补 {participants.filter(p => p.status === 'waitlist').length}
              </span>
              <span className="text-red-600">
                已取消 {participants.filter(p => p.status === 'cancelled').length}
              </span>
            </div>
          </div>
