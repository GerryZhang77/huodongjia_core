...
const MatchingConfiguration: React.FC = () => {
  ...
  const executeMatching = async () => {
    ...
    try {
      ...
      const similarityResponse = await fetch(`/api/calculate-similarity/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
    ...
    groups.forEach((group, groupIndex) => {
      ...
      group.members.forEach((member, memberIndex) => {
        ...
        bubbles.push({
          ...
          color: groupColor,
          connections: group.members.filter(m => m.user_id !== member.user_id).map(m => m.user_id)
        })
      })
    })
  ...
  // 交换成员
  const swapMembers = (fromGroupId: string, toGroupId: string, memberId: string) => {
    setMatchingGroups(prev => {
      ...
      if (fromGroup && toGroup && !fromGroup.is_locked && !toGroup.is_locked) {
        ...
        if (member) {
          fromGroup.members = fromGroup.members.filter(m => m.user_id !== memberId)
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Tag
                  color="primary"
                  fill="outline"
                  onClick={() => setNaturalLanguageInput('希望按照兴趣爱好和专业技能进行分组，让有共同话题的人组成团队')}
                >
                  兴趣匹配
                </Tag>
                <Tag
                  color="primary"
                  fill="outline"
                  onClick={() => setNaturalLanguageInput('根据工作经验和行业背景进行匹配，促进跨领域交流')}
                >
                  经验匹配
                </Tag>
                <Tag
                  color="primary"
                  fill="outline"
                  onClick={() => setNaturalLanguageInput('平衡各组的性别比例、年龄结构和地域分布')}
                >
                  平衡分组
                </Tag>
              </div>
          ...
          <Card title="执行智能匹配" style={{ '--border-radius': '12px' }}>
            <div className="space-y-4">
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">准备执行匹配</h3>
                <p className="text-gray-600 mb-6">
                  将基于您设置的规则对 {participants.length} 位参与者进行智能匹配
                </p>
                ...
                {isMatching && (
                  <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-2">
                      ...
                      {matchingProgress >= 20 && matchingProgress < 40 && '正在提取关键词...'}
                      {matchingProgress >= 40 && matchingProgress < 60 && '正在计算词嵌入...'}
                      {matchingProgress >= 60 && matchingProgress < 80 && '正在计算相似度...'}
                ...
                <Button
                  ...
                  disabled={generatedRules.filter(r => r.enabled).length === 0}
                  style={{
                    '--border-radius': '12px',
                    height: '48px',
                  }}
          ...
          <div className="space-y-4">
            ...
            <Card title="匹配结果" style={{ '--border-radius': '12px' }}>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-blue-500">{matchingGroups.length}</div>
                  <div className="text-sm text-gray-600">分组数量</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-green-500">
                    {Math.round(matchingGroups.reduce((sum, group) => sum + group.similarity_score, 0) / matchingGroups.length * 100)}%
                  </div>
                    ...
                    <svg width="100%" height="100%" viewBox="0 0 400 300">
                      ...
                      {bubbleData.map(bubble => 
                        bubble.connections.map(connectionId => {
                          const target = bubbleData.find(b => b.id === connectionId)
                      ...
                      {bubbleData.map(bubble => (
                        <g key={bubble.id}>
                          ...
                          <text
                            ...
                            fontWeight="500"
                          >
                            {bubble.name.length > 4 ? bubble.name.substring(0, 3) + '...' : bubble.name}
                          </text>
                        </g>
                      ))}
                    </svg>
                      ...
                      matchingGroups.map((group, groupIndex) => (
                        <Card
                          ...
                          style={{
                            '--border-radius': '12px',
                            '--body-padding': '16px',
                            border: group.is_locked ? '2px solid #1890ff' : '1px solid #f0f0f0'
                          }}
                        >
                          {/* 锁定按钮 */}
                          <Button
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <Badge
                                content={`${Math.round(group.similarity_score * 100)}% 匹配`}
                                color="#52c41a"
                              />
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {group.members.map((member) => (
                                <div key={member.user_id} className="text-center">
                                  <Avatar
                                    src={member.avatar}
                                    style={{ '--size': '48px' }}
                                    className="mb-1"
                                  ...
                                  <div className="flex flex-col gap-1 mt-1">
                                    {member.keywords?.slice(0, 2).map((keyword, index) => (
                                      <Tag
                                        key={index}
                                        color="primary"
                                        fill="outline"
                            ...
                            <div className="pt-2 border-t border-gray-100">
                              <div className="text-sm text-gray-600 mb-2">匹配理由：</div>
